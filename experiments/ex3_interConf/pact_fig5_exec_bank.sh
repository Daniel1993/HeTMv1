...
function run_may_fail {
	echo "timeout 40s ./bank -n $CPU_THREADS -b ${GPU_BLOCKS} -x ${GPU_THREADS} -T 1 -a $DATASET -d $DURATION \
	...
	#	-R 0 -S $TRANSACTION_SIZE -l ${1} -N 1 CPU_BACKOFF=$CPU_BACKOFF GPU_BACKOFF=$GPU_BACKOFF -X 0.08 >/dev/null
...
function doRunLargeDTST_1GPU {
	...
	do
		# 100M 500M 1G 1.5G
		echo "./compile.sh opt CMP_TYPE=COMPRESSED LOG_TYPE=BMAP USE_TSX_IMPL=$USE_TSX PR_MAX_RWSET_SIZE=50 \
		...
		LOG_SIZE=4096 STM_LOG_BUFFER_SIZE=256 BANK_PART_SCALE=1  DISABLE_EARLY_VALIDATION=1 HETM_NB_DEVICES=1"
		./compile.sh opt CMP_TYPE=COMPRESSED LOG_TYPE=BMAP USE_TSX_IMPL=$USE_TSX PR_MAX_RWSET_SIZE=50 \
			BANK_PART=9 BANK_INTRA_CONFL=0 GPU_PART=0.55 CPU_PART=0.55 P_INTERSECT=0.00 PROFILE=1 -j 14 \
			BMAP_GRAN_BITS=13 DISABLE_NON_BLOCKING=1 OVERLAP_CPY_BACK=0 \
			LOG_SIZE=4096 STM_LOG_BUFFER_SIZE=256 BANK_PART_SCALE=1  DISABLE_EARLY_VALIDATION=1 HETM_NB_DEVICES=1 2>/dev/null 1>/dev/null
